 ...
 */
#ifndef __QUADPROG___CC__
#define __QUADPROG___CC__
#include <iostream>
#include <algorithm>
#include <cmath>
#include <limits>
#include <sstream>
#include <stdexcept>
#include "barrier_avoid/QuadProg++.hpp"
//#define TRACE_SOLVER
 ...
 *      I*x <= i0;
 */
std::vector<double> my_quadprog(std::vector<double>& G_in, std::vector<double>& g0_in, 
                      std::vector<double>& CE_in, std::vector<double>& ce0_in,  
                      std::vector<double>& CI_in, std::vector<double>& ci0_in)
{
    ...
    double sum = 0.0;
    char ch;
    n = sqrt(G_in.size()); //G , n*n
    m = CE_in.size()/n; //CE, n*m
    p = CI_in.size()/n; //CI, n*p
    G.resize(n, n);{
        for (int i = 0; i < n; i++) 
            for (int j = 0; j < n; j++)
                G[i][j] = G_in[i*n+j];
    ...
    res.push_back(solve_quadprog(G, g0, CE, ce0, CI, ci0, x));
