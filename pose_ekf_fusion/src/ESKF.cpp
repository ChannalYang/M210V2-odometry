...
namespace eskf{
 ...
 void ESKF::initialiseCovariance() {
  ...
  //  std::cout << "corrected_delta_ang" << corrected_delta_ang[0] << "\t" << corrected_delta_ang[1] << "\t" << corrected_delta_ang[2] << std::endl;
    std::cout << "corrected_delta_vel" << corrected_delta_vel[0] << "\t" << corrected_delta_vel[1] << "\t" << corrected_delta_vel[2] << std::endl;
 //   std::cout << "corrected_delta_ang" << corrected_delta_ang[0] << "\t" << corrected_delta_ang[1] << "\t" << corrected_delta_ang[2] << std::endl;
    std::cout << "vel_last" << vel_last[0] << "\t" << vel_last[1] << "\t" << vel_last[2] << std::endl;
 ...
 //   state_.vel(2) += kOneG * imu_sample_delayed_.delta_vel_dt;
  ...
  void ESKF::predictCovariance() {
    ...
    process_noise[18] = process_noise[17] = process_noise[16] = 0.0;
    // body frame magnetic field states
    process_noise[21] = process_noise[20] = process_noise[19] = 0.0;
  ...
  void ESKF::controlOpticalFlowFusion() {
    ...
    if(flow_data_ready_) {
      ...
      if ((fusion_mask_ & MASK_OPTICAL_FLOW) && (!opt_flow_)) {
        opt_flow_ = true;
        std::cout << "ESKF commencing optical flow fusion" << endl;
      }
      ...
      if (range_data_ready_) {
        ...
        if ((fusion_mask_ & MASK_RANGEFINDER) && (!rng_hgt_)) {
          if (time_last_imu_ - time_last_range_ < 2 * RANGE_MAX_INTERVAL) {
            rng_hgt_ = true;
            std::cout << "ESKF commencing rng fusion" << endl;
          }
        }
    ...
    if(vision_data_ready_) {
      ...
      if ((fusion_mask_ & MASK_EV_POS) && (!ev_pos_)) {
        ...
        if (time_last_imu_ - time_last_ext_vision_ < 2 * EV_MAX_INTERVAL) {
          ev_pos_ = true;
          std::cout << "ESKF commencing external vision position fusion" << endl;
        }
      ...
      if ((fusion_mask_ & MASK_EV_YAW) && (!ev_yaw_)) {
        if (time_last_imu_ - time_last_ext_vision_ < 2 * EV_MAX_INTERVAL) {
          ...
          ev_yaw_ = true;
          std::cout <<"ESKF commencing external vision yaw fusion"<<endl;
        }
      }
      ...
      if ((fusion_mask_ & MASK_EV_HGT) && (!ev_hgt_)) {
        ...
        if (time_last_imu_ - time_last_ext_vision_ < 2 * EV_MAX_INTERVAL) {
          ev_hgt_ = true;
          std::cout<<"ESKF commencing external vision hgt fusion" <<endl;
          if(rng_hgt_) {
            //
          } else {
            resetHeight();
  ...
  void ESKF::controlGpsFusion() {
    if (gps_data_ready_) {
      if ((fusion_mask_ & MASK_GPS_POS) && (!gps_pos_)) {
        gps_pos_ = true;
        std::cout<<"ESKF commencing GPS pos fusion" << endl;
      }
      if(gps_pos_) {
        fuse_pos_ = true;
        fuse_vert_vel_ = true;
      ...
      if ((fusion_mask_ & MASK_GPS_HGT) && (!gps_hgt_)) {
        gps_hgt_ = true;
        std::cout<<"ESKF commencing GPS hgt fusion" << endl;
      }
      if(gps_pos_) {
        fuse_height_ = true;
        time_last_gps_ = time_last_imu_;
      ...
      if ((time_last_imu_ - time_last_fake_gps_ > (uint64_t)2e5) || fuse_height_) {
        ...
        if ((time_last_imu_ - time_last_fake_gps_) > (uint64_t)4e5) {
          ...
          if (time_last_fake_gps_ != 0) {
            std::cout << "ESKF stopping navigation" << endl;
          }
        }
  ...
  void ESKF::updateOpticalFlow(const vec2& int_xy, const vec2& int_xy_gyro, uint32_t integration_time_us, scalar_t distance, uint8_t quality, uint64_t time_us, scalar_t dt) {
    ...
    if (time_us - time_last_opt_flow_ > min_obs_interval_us_) {
      ...
      if ((delta_time_good && flow_quality_good && (relying_on_flow)) || !in_air_) {
  ...
  //    scalar_t pitch = atan2f(-R_to_earth_(2, 0), R_to_earth_(2, 2)); // third rotation (pitch)
      ...
      if(ev_yaw_) {
        ...
        measured_hdg = atan2(Tbn_0_1_neg, Tbn_1_1);
    ...
    if (heading_innov_var >= R_YAW) {
      ...
      // we reinitiafabsflise the covariance matrix and abort this fusion step
      initialiseCovariance();
      return;
    }
    ...
    if (yaw_test_ratio > 1.0f) {
      if(in_air_)
        ...
        scalar_t gate_limit = sqrt(sq(heading_innov_gate_) * heading_innov_var);
  ...
  void ESKF::fixCovarianceErrors() {
    ...
    scalar_t minAllowedStateVar = fmax(0.01f * maxStateVar, minStateVarTarget);
  ...
  void ESKF::initialiseQuatCovariances(const vec3& rot_vec_var) {
    ...
    scalar_t delta = 2.0f*acos(q0);
    scalar_t scaler = (delta/sin(delta*0.5f));
    ...
    if (t6 > 1e-9f) {
      ...
      scalar_t t11 = 1.0f/sqrt(t6);
      scalar_t t12 = cos(t8);
      scalar_t t13 = 1.0f/pow(t6,1.5f);
      scalar_t t14 = t3*t11;
