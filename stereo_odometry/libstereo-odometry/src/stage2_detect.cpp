		...
		bool													update_dyn_thresholds )
{
    ...
    std::cout<<"H:"<<img_data.pyr.images[0].getSize()<< img_data.pyr.images.size()<<std::endl;
	...
	for( size_t octave = 0; octave < nOctaves; ++octave )
	{
        ...
        //std::cout<<input_im.cols<<" "<<input_im.rows<<std::endl;
		...
		else if( params_detect.detect_method == TDetectParams::dmFASTER )
		{
         ...
         std::cout<<"features size:   "<<feats_vector.size()<<std::endl;
             std::cout<<desc_aux.cols<<" "<<desc_aux.rows<<std::endl;
        ...
        std::cout<<img_data.pyr_feats_kps[0].size()<<std::endl;

        cv::Mat aaaa = img_data.pyr_feats_desc[0];

        std::cout<<aaaa.cols<<" "<<aaaa.rows<<std::endl;
