		...
		CStereoOdometryEstimator::TImagePairData & cur_imgpair )					// not const because the ids of the matches will change!
{
	...
	if( params_if_match.ifm_method == TInterFrameMatchingParams::ifmDescBF )
	{
		...
		for( size_t octave = 0; octave < nOctaves; ++octave )
		{
            ...
            vector<cv::Point2f> pi1(matL.size()),pi2(matL.size());
            ...
            for( k = 0, itM = matL.begin(); itM != matL.end(); ++itM, ++k )
			{
                ...
                pi1[k].x = p1.at<float>(k,0);
                pi1[k].y = p1.at<float>(k,1);
                pi2[k].x = p2.at<float>(k,0);
                pi2[k].y = p2.at<float>(k,1);
                ...
                // std::cout << p1.at<float>(k,0) << " " << p1.at<float>(k,1) << "\t";
                // std::cout << p2.at<float>(k,0) << " " << p2.at<float>(k,1) << endl;
          ...
          //  std::cout << "sweat heart 2 p1:"<< p1.rows<< "p2:" << p2.rows<< std::endl;
         //   std::cout << "sweat heart 2 p1:"<< p1.cols<< "p2:" << p2.cols<< std::endl;
            cv::findFundamentalMat(pi1,pi2,cv::FM_RANSAC,3,0.99,inliersLeft);
           // std::cout << "sweat heart 2 p1:"<< p1.rows<< "p2:" << p2.rows<< std::endl;
