...
//    } // end-if
  ...
  //  cout<<"residual:  "<<result.out_residual <<endl;
    ...
    for( size_t m = 0, i = 0; m < nL; ++m)
    {
		...
		const cv::KeyPoint & featR = list1_r[m];
         ...
         double vl  = featL.pt.y;
         double ur  = featR.pt.x;
         double b_d = baseline/(fl*(cur-ur)+fr*(ul-cul));
        lmks[i] = TPoint3D(b_d*fr*(ul-cul),b_d*fr*(vl-cvl),b_d*fl*fr);				// (X,Y,Z)
        // cout<<  "link[i]:  "<<  lmks[i].x <<endl;
        // cout<<   "true:    "<<  TPoint3D(b_d*fr*(ul-cul),b_d*fr*(vl-cvl),b_d*fl*fr).x <<endl;
		++i;
    } // end for m
    ...
    {
      //  cout<<"out_residual"<<i<<out_residual[i] <<endl;
